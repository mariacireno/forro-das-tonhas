export function Bandeirinhas({ count = 8, colors, height = 64, style = {} }) {
...
export function Star({ size = 32, fill = 'var(--yellow)', stroke = 'var(--ink)', strokeWidth = 2, rotate = 0, style = {} }) {
  const r = 50
  const points = []
  for (let i = 0; i < 10; i++) {
...
export function Sparkle({ size = 24, fill = 'var(--indigo)', style = {} }) {
  return (
    <svg viewBox="0 0 100 100" width={size} height={size} style={style} aria-hidden="true">
      <path d="M50 0 L60 40 L100 50 L60 60 L50 100 L40 60 L0 50 L40 40 Z" fill={fill} />
    </svg>
  )
}

export function Sunburst({ size = 160, points = 14, fill = 'var(--yellow)', stroke = 'var(--ink)', strokeWidth = 2, style = {}, children }) {
  const path = []
  const cx = 50, cy = 50
  const rOuter = 50
  const rInner = 36
...
export function SunRays({ size = 200, fill = 'var(--yellow)', stroke = 'var(--ink)', rays = 8, style = {} }) {
  const segments = []
...
export function Sanfona({ width = 220, color = 'var(--indigo)', style = {} }) {
  return (
...
export function DancingCouple({ width = 220, color = 'var(--indigo)', style = {} }) {
  return (
    <svg viewBox="0 0 220 240" width={width} style={{ display: 'block', ...style }} aria-hidden="true">
      <g fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
export function Cloud({ width = 80, fill = 'var(--cream-deep)', style = {} }) {
  return (
