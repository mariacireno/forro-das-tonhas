...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#F7F2E2;border:2.5px solid #16143A;border-radius:18px;overflow:hidden;box-shadow:0 4px 0 0 #16143A;">
  ...
  ${qrItems.length > 0 ? `<tr><td style="padding:20px 28px 0;">
    <div style="font-family:'Arial Black',Arial,sans-serif;font-size:12px;color:#16143A;text-transform:uppercase;letter-spacing:0.14em;font-weight:700;margin-bottom:12px;">🎟️ Seus ingressos</div>
    ${qrItems.map(q => `
    <table cellpadding="0" cellspacing="0" style="width:100%;background:#F1ECDB;border:2px solid #16143A;border-radius:14px;overflow:hidden;margin-bottom:10px;">
      ...
      <tr><td style="padding:14px;text-align:center;">
        <img src="${q.qrDataUrl}" alt="QR Code" width="160" height="160" style="display:block;margin:0 auto;border:2px solid #16143A;border-radius:8px;" />
...
async function sendConfirmacaoIngresso(venda, checkins = []) {
  ...
  // Gera QR codes para o HTML (data URLs inline)
  const qrItems = await Promise.all(checkins.map(async (c) => ({
    tipo: c.tipo,
    qrDataUrl: await QRCode.toDataURL(`checkin:${c.id}`, { width: 200, margin: 1, color: { dark: '#16143A', light: '#F7F2E2' } }),
  })))
